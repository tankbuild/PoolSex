...
class IndelRealigner():
    ...
    def __init__(self, data):
        self.qsub_file_path = os.path.join(data.directories.qsub, 'gatk_indel_realigner.sh')
    ...
    def generate_shell_files(self, data, parameters, sex):
        ...
        base_file_name = sex
        base_shell_name = 'gatk_indel_realigner_' + base_file_name
        shell_file_path = os.path.join(data.directories.shell, base_shell_name + '.sh')
        shell_file = open(shell_file_path, 'w')
        output_file_path = os.path.join(data.directories.results, base_file_name + '_realigned.bam')
